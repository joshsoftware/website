...
const Feed = props => {
  ...
  let date = new Date(pubDate.split(" ")[0]);
  ...
  return (
    <li className="feed mb-2" key={title}>
      <img
        src={thumbnail}
        className="img-feed align-self-start mr-3"
        alt="Teaser"
      />
      <div className="feed-body">
        ...
        <h6 className="font-weight-normal mb-3">
          {author},{" "}
          <small className="font-weight-light">{date.toDateString()}</small>
        </h6>

        <div
          dangerouslySetInnerHTML={{ __html: sanitizer(shortBody) }}
